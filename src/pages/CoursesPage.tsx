  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold dark:text-white">{user?.role === 'student' ? 'My Courses' : 'Course Management'}</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
            />
          </div>
          <select value={levelFilter} onChange={e => setLevelFilter(e.target.value)} className="px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
          ...
          <select value={semFilter} onChange={e => setSemFilter(e.target.value)} className="px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((c, idx) => {
          ...
          return (
            <div key={c.id} onClick={() => onNavigate('courseDetail')} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all cursor-pointer overflow-hidden group">
              ...
              <div className="p-4">
                <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mb-3">{c.description}</p>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600 dark:text-gray-400">{c.lecturerName}</span>
                  </div>
                  {user?.role === 'student' && (
