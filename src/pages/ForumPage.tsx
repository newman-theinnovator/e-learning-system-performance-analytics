  ...
  if (post) {
    return (
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-4 sm:p-6">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-semibold dark:text-white">{post.authorName}</span>
              ...
              <h2 className="text-xl font-bold mt-2 dark:text-white">{post.title}</h2>
              <p className="text-sm text-gray-700 dark:text-gray-300 mt-3 whitespace-pre-line leading-relaxed">{post.content}</p>
        ...
        <div className="space-y-3">
          ...
          {post.replies.map(r => (
            <div key={r.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-3 sm:p-4 ml-3 sm:ml-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm dark:text-white">{r.authorName}</span>
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">{r.content}</p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-3 sm:p-4 ml-3 sm:ml-6">
            <textarea className="w-full p-3 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:ring-2 focus:ring-blue-500" rows={3} placeholder="Write a reply..." />
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold dark:text-white">Discussion Forum</h1>
          <p className="text-gray-500 dark:text-gray-400 text-xs sm:text-sm">{forumPosts.length} discussions</p>
        </div>
      ...
      <div className="space-y-3">
        {forumPosts.map(p => (
          <div key={p.id} onClick={() => setSelectedPost(p.id)} className={`bg-white dark:bg-gray-800 rounded-xl border shadow-sm p-4 sm:p-5 cursor-pointer hover:shadow-md transition-all ${p.isPinned ? 'border-amber-200 dark:border-amber-700 bg-amber-50/20 dark:bg-amber-900/10' : 'border-gray-100 dark:border-gray-700'}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-semibold mt-1 dark:text-white">{p.title}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{p.content}</p>
